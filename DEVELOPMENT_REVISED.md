...
当前实施进度：桌面壳、SQLite 基础数据层、角色/对话创建、Mock 流式聊天以及 OpenAI Compatible Provider 基础能力已经可用。Phase 3 已具备停止生成、模型参数设置和经过测试的超时/取消语义；完整 SDK 事件到领域 StreamEvent 的转换仍待完成。角色卡导入、Prompt Engine、世界书等后续能力仍按下列路线推进。
...
- [x] 能配置自定义 baseUrl；
- [x] 能选择模型；
- [x] 能配置并持久化温度、最大输出 Token 和请求超时；
- [x] 能流式输出；
- [x] 停止生成能生效；
- [x] 用户停止与请求超时可区分，且都不会保存未完成的 assistant 回复；
- [x] 错误能显示给用户。
